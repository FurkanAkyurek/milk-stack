...
public class ScreenShooterWindow : EditorWindow {
	...
	private void OnGUI() {
		...
		if (GUILayout.Button("Capture Screenshots Series")) {
			ScreenshotTakerEditor.CaptureScreenshootQueueAllLanguages(data.screenshoots, "");
		}
	...
	private string PathField(string label, string path) {
		...
		if (GUILayout.Button("Open", GUILayout.Width(100))) {
			var file = Directory.EnumerateFiles("").FirstOrDefault();
			if (!string.IsNullOrEmpty(file))
				EditorUtility.RevealInFinder(Path.Combine("", file));
			else
