...
namespace WSMGameStudio.Splines
{
    public class SplineFollower : MonoBehaviour
    {
        ...
        [System.NonSerialized] private int _currentSplineIndex = 0;
        [System.NonSerialized] private int _lastSplineIndex = 0;
        ...
        [System.NonSerialized] private OrientedPoint[][] _normalizedOrientedPoints;
        ...
        /// </summary>
        private void NormalizeOrientedPoints()
        {
            if (splines == null || splines.Count == 0)
                return;
        ...
        {
            ValidateOrientedPoints(lastSplineIndex);
            ValidateOrientedPoints(currentSplineIndex);
            float newSplineRatio = CalculateSplineTransitionRatio(lastSplineIndex, currentSplineIndex);
            float newSplineProgress = _goingForward ? ((_splineProgress - 1f) * newSplineRatio) : (1f - (Mathf.Abs(_splineProgress) * newSplineRatio));
        ...
        {
            int ret = 0;
            ValidateOrientedPoints(_currentSplineIndex);
        ...
        /// <summary>
        /// Cnovert speed to selected Speed Unit
        /// </summary>
        /// <returns></returns>
        private float GetConvertedSpeedUnit()
        {
        ...
        /// </summary>
        private void ValidateOrientedPoints(int splineIndex)
        {
        ...
        private int CalculateLinkedFollowersAcumulatedOffset(int index, int acumulatedOffset)
        {
            acumulatedOffset = index == 0 ? followersOffset + (linkedFollowers[index].followingOffset / 2) : (acumulatedOffset + (linkedFollowers[index].followingOffset / 2) + (linkedFollowers[index - 1].followingOffset / 2));
